...
public final class SplitRuleParser {
    private final ComponentName buildClassName(String pkg, CharSequence clsSeq) {
        if (clsSeq != null) {
            if (!(clsSeq.length() == 0)) {
                ...
                if (string.charAt(0) == '.') {
                    return new ComponentName(pkg, p.t(pkg, string));
                }
                int iQ0 = d0.q0(string, '/', 0, false, 6, null);
                if (iQ0 > 0) {
                    pkg = string.substring(0, iQ0);
                    p.j(pkg, "this as java.lang.String…ing(startIndex, endIndex)");
                    string = string.substring(iQ0 + 1);
                    p.j(string, "this as java.lang.String).substring(startIndex)");
                }
                if (p.f(string, "*") || d0.q0(string, '.', 0, false, 6, null) >= 0) {
    ...
    private final ActivityFilter parseActivityFilter(Context context, XmlResourceParser parser) {
        ...
        p.j(packageName, HandleInvocationsFromAdViewer.KEY_PACKAGE_NAME);
    ...
    private final ActivityRule parseSplitActivityRule(Context context, XmlResourceParser parser) {
        return new ActivityRule(w0.f(), context.getTheme().obtainStyledAttributes(parser, R.styleable.ActivityRule, 0, 0).getBoolean(R.styleable.ActivityRule_alwaysExpand, false));
    }
    ...
    private final SplitPlaceholderRule parseSplitPlaceholderRule(Context context, XmlResourceParser parser) {
        ...
        p.j(component, "Intent().setComponent(pl…eholderActivityClassName)");
        return new SplitPlaceholderRule(setF, component, dimension, dimension2, f10, i10);
    }
    ...
    private final Set<EmbeddingRule> parseSplitXml(Context context, int splitResourceId) throws XmlPullParserException, IOException {
        ...
        try {
            ...
            p.j(xml, "resources.getXml(splitResourceId)");
            ...
            while (next != 1 && (next != 3 || xml.getDepth() > depth)) {
                if (xml.getEventType() != 2 || p.f("split-config", xml.getName())) {
                    next = xml.next();
                } else {
