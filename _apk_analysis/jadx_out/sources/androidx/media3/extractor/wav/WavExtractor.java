    ...
    public static final class ImaAdPcmOutputWriter implements OutputWriter {
        ...
        public ImaAdPcmOutputWriter(ExtractorOutput extractorOutput, TrackOutput trackOutput, WavFormat wavFormat) throws ParserException {
            ...
            if (littleEndianUnsignedShort == i11) {
                ...
                int i12 = ((wavFormat.frameRateHz * wavFormat.blockSize) * 8) / littleEndianUnsignedShort;
                this.format = new Format.Builder().setSampleMimeType("audio/raw").setAverageBitrate(i12).setPeakBitrate(i12).setMaxInputSize(numOutputFramesToBytes(iMax, i10)).setChannelCount(wavFormat.numChannels).setSampleRate(wavFormat.frameRateHz).setPcmEncoding(2).build();
                return;
            }
        ...
        public void init(int i10, long j10) {
            this.extractorOutput.seekMap(new WavSeekMap(this.wavFormat, this.framesPerBlock, i10, j10));
    ...
    public static final class PassthroughOutputWriter implements OutputWriter {
        ...
        public PassthroughOutputWriter(ExtractorOutput extractorOutput, TrackOutput trackOutput, WavFormat wavFormat, String str, int i10) throws ParserException {
            ...
            if (wavFormat.blockSize == i11) {
                ...
                this.format = new Format.Builder().setSampleMimeType(str).setAverageBitrate(i13).setPeakBitrate(i13).setMaxInputSize(iMax).setChannelCount(wavFormat.numChannels).setSampleRate(wavFormat.frameRateHz).setPcmEncoding(i10).build();
                return;
            }
            throw ParserException.createForMalformedContainer("Expected block size: " + i11 + "; got: " + wavFormat.blockSize, null);
        }

        @Override // androidx.media3.extractor.wav.WavExtractor.OutputWriter
        public void init(int i10, long j10) {
            this.extractorOutput.seekMap(new WavSeekMap(this.wavFormat, 1, i10, j10));
            this.trackOutput.format(this.format);
        }
