...
public final class AndroidClipboardManager_androidKt {
    ...
    public static final AnnotatedString convertToAnnotatedString(@Nullable CharSequence charSequence) {
        ...
        tn.p.j(annotationArr, "annotations");
        int iC0 = cn.r.c0(annotationArr);
        if (iC0 >= 0) {
            while (true) {
                Annotation annotation = annotationArr[i10];
                if (tn.p.f(annotation.getKey(), "androidx.compose.text.SpanStyle")) {
                    int spanStart = spanned.getSpanStart(annotation);
                    int spanEnd = spanned.getSpanEnd(annotation);
                    String value = annotation.getValue();
                    tn.p.j(value, "span.value");
