package yads;

import android.content.Context;
import android.view.View;
import android.view.Window;
import android.webkit.WebView;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ProgressBar;
import android.widget.RelativeLayout;
import android.widget.TextView;
import com.ironsource.C3978d4;

/* JADX INFO: loaded from: classes2.dex */
public final class t1 implements h2, t3 {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    public final Context f95031a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    public final RelativeLayout f95032b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    public final f2 f95033c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    public final Window f95034d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    public final String f95035e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    public final u3 f95036f;

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    public final LinearLayout f95037g;

    /* JADX INFO: renamed from: h, reason: collision with root package name */
    public final TextView f95038h;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public final ProgressBar f95039i;

    /* JADX INFO: renamed from: j, reason: collision with root package name */
    public final db3 f95040j;

    public t1(Context context, RelativeLayout relativeLayout, q2 q2Var, Window window, String str, u3 u3Var, LinearLayout linearLayout, TextView textView, ProgressBar progressBar, db3 db3Var) {
        this.f95031a = context;
        this.f95032b = relativeLayout;
        this.f95033c = q2Var;
        this.f95034d = window;
        this.f95035e = str;
        this.f95036f = u3Var;
        this.f95037g = linearLayout;
        this.f95038h = textView;
        this.f95039i = progressBar;
        this.f95040j = db3Var;
    }

    public static final void a(t1 t1Var, View view) {
        String url = t1Var.f95036f.getUrl();
        if (url != null) {
            t1Var.f95040j.a(t1Var.f95031a, url);
        }
    }

    public static final void b(t1 t1Var, View view) {
        ((q2) t1Var.f95033c).f93865a.finish();
    }

    @Override // yads.h2
    public final void a() {
        u3 u3Var = this.f95036f;
        u3Var.getClass();
        int i10 = tb.f95131b;
        try {
            WebView.class.getDeclaredMethod(C3978d4.i.f31366u0, new Class[0]).invoke(u3Var, new Object[0]);
        } catch (Exception unused) {
        }
        u3.class.toString();
        boolean z10 = ad1.f87661a;
    }

    public final void a(int i10) {
        if (i10 == 0 && this.f95039i.getVisibility() != 0) {
            this.f95039i.bringToFront();
            this.f95032b.requestLayout();
            this.f95032b.invalidate();
        }
        this.f95039i.setVisibility(i10);
    }

    public final void a(ImageView imageView, ImageView imageView2) {
        imageView.setOnClickListener(new View.OnClickListener() { // from class: bt.ba
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                yads.t1.a(this.f5959b, view);
            }
        });
        imageView2.setOnClickListener(new View.OnClickListener() { // from class: bt.ca
            @Override // android.view.View.OnClickListener
            public final void onClick(View view) {
                yads.t1.b(this.f5974b, view);
            }
        });
    }

    @Override // yads.h2
    public final void b() {
        u3 u3Var = this.f95036f;
        u3Var.getClass();
        int i10 = tb.f95131b;
        try {
            WebView.class.getDeclaredMethod(C3978d4.i.f31364t0, new Class[0]).invoke(u3Var, new Object[0]);
        } catch (Exception unused) {
        }
        u3.class.toString();
        boolean z10 = ad1.f87661a;
    }

    @Override // yads.h2
    public final void c() {
        this.f95032b.setBackgroundDrawable(q9.f93954a);
        LinearLayout linearLayout = this.f95037g;
        Context context = this.f95031a;
        t9 t9Var = t9.f95107e;
        ImageView imageViewA = r9.a(context, "data:image/png;base64,", kl3.a(context, t9Var.a(context) - 2));
        Context context2 = this.f95031a;
        ImageView imageViewA2 = r9.a(context2, "data:image/png;base64,", kl3.a(context2, t9Var.a(context2)));
        imageViewA2.setTag(hl3.a("browser_close_button"));
        a(imageViewA, imageViewA2);
        linearLayout.addView(this.f95038h, new LinearLayout.LayoutParams(0, -1, 1.0f));
        linearLayout.addView(imageViewA, new LinearLayout.LayoutParams(-2, -1));
        linearLayout.addView(imageViewA2, new LinearLayout.LayoutParams(-2, -1));
        RelativeLayout relativeLayout = this.f95032b;
        LinearLayout linearLayout2 = this.f95037g;
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-1, kl3.a(this.f95031a, t9.f95106d.a(r2)));
        layoutParams.addRule(10);
        relativeLayout.addView(linearLayout2, layoutParams);
        Context context3 = this.f95031a;
        LinearLayout linearLayout3 = this.f95037g;
        RelativeLayout.LayoutParams layoutParams2 = new RelativeLayout.LayoutParams(-1, kl3.a(context3, 2.0f));
        layoutParams2.addRule(3, linearLayout3.getId());
        this.f95032b.addView(this.f95039i, layoutParams2);
        a(8);
        RelativeLayout relativeLayout2 = this.f95032b;
        u3 u3Var = this.f95036f;
        LinearLayout linearLayout4 = this.f95037g;
        RelativeLayout.LayoutParams layoutParams3 = new RelativeLayout.LayoutParams(-1, -1);
        layoutParams3.addRule(3, linearLayout4.getId());
        relativeLayout2.addView(u3Var, layoutParams3);
        this.f95036f.loadUrl(this.f95035e);
        ((q2) this.f95033c).a(6, null);
        boolean z10 = ad1.f87661a;
    }

    @Override // yads.h2
    public final boolean d() {
        boolean z10;
        if (this.f95036f.canGoBack()) {
            u3 u3Var = this.f95036f;
            if (u3Var.canGoBack()) {
                u3Var.goBack();
            }
            z10 = true;
        } else {
            z10 = false;
        }
        return !z10;
    }

    @Override // yads.h2
    public final void f() {
        this.f95034d.requestFeature(1);
        if (ub.a(16)) {
            this.f95034d.getDecorView().setSystemUiVisibility(256);
        }
    }

    @Override // yads.h2
    public final void g() {
        this.f95036f.c();
    }

    @Override // yads.h2
    public final void onAdClosed() {
        ((q2) this.f95033c).a(8, null);
    }
}
