package com.ironsource;

import android.annotation.SuppressLint;
import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebResourceResponse;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import androidx.appcompat.widget.ActivityChooserModel;
import com.ironsource.Qf;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import com.mbridge.msdk.MBridgeConstans;
import com.safedk.android.analytics.brandsafety.creatives.CreativeInfoManager;
import com.safedk.android.internal.SafeDKWebAppInterface;
import com.safedk.android.internal.partials.IronSourceNetworkBridge;
import com.safedk.android.utils.Logger;
import com.unity3d.mediation.LevelPlay;
import com.vungle.ads.internal.ui.AdActivity;
import java.util.List;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes7.dex */
public final class Qf implements Gf {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    @NotNull
    private final Handler f30312a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    @Nullable
    private WebView f30313b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    @NotNull
    private final WebView f30314c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    @NotNull
    private final JSONObject f30315d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    private boolean f30316e;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    private boolean f30317f;

    public static final class a extends WebViewClient {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        public final /* synthetic */ String f30319b;

        public a(String str) {
            this.f30319b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onLoadResource(WebView webView, String str) {
            super.onLoadResource(webView, str);
            CreativeInfoManager.onResourceLoaded(com.safedk.android.utils.g.f53132f, webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(@Nullable WebView webView, @Nullable String str) throws JSONException {
            Logger.d("IronSource|SafeDK: Execution> Lcom/ironsource/Qf$a;->onPageFinished(Landroid/webkit/WebView;Ljava/lang/String;)V");
            CreativeInfoManager.onWebViewPageFinished(com.safedk.android.utils.g.f53132f, webView, str);
            safedk_Qf$a_onPageFinished_56d2903bcd1431312f16a56119e75d08(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(@NotNull WebView webView, @NotNull WebResourceRequest webResourceRequest, @NotNull WebResourceError webResourceError) throws JSONException {
            tn.p.k(webView, MBridgeConstans.DYNAMIC_VIEW_KEY_VIEW);
            tn.p.k(webResourceRequest, AdActivity.REQUEST_KEY_EXTRA);
            tn.p.k(webResourceError, "error");
            Bf.f28863a.a(webResourceError.toString());
            Kf.f29735a.b("got error during TestSuite web controller loading " + webResourceError);
            super.onReceivedError(webView, webResourceRequest, webResourceError);
        }

        public void safedk_Qf$a_onPageFinished_56d2903bcd1431312f16a56119e75d08(WebView webView, String str) throws JSONException {
            if (!Qf.this.f30316e && str != null && bo.a0.W(str, String.valueOf(this.f30319b), false, 2, null)) {
                Qf qf2 = Qf.this;
                List<? extends Object> listE = cn.v.e(qf2.f30315d);
                tn.p.i(listE, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                qf2.a(qf2.a("onDataReady", listE));
                Bf.f28863a.d();
                Qf.this.f30316e = true;
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public WebResourceResponse shouldInterceptRequest(WebView webView, WebResourceRequest webResourceRequest) {
            return CreativeInfoManager.onWebViewResponseWithHeaders(com.safedk.android.utils.g.f53132f, webView, webResourceRequest, super.shouldInterceptRequest(webView, webResourceRequest));
        }

        @Override // android.webkit.WebViewClient
        public WebResourceResponse shouldInterceptRequest(WebView webView, String str) {
            return CreativeInfoManager.onWebViewResponse(com.safedk.android.utils.g.f53132f, webView, str, super.shouldInterceptRequest(webView, str));
        }
    }

    public static final class b implements V8 {

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        public final /* synthetic */ V8 f30321b;

        public b(V8 v82) {
            this.f30321b = v82;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public static final void a(b bVar) {
            tn.p.k(bVar, "this$0");
            bVar.onUIReady();
        }

        @Override // com.ironsource.V8
        public void onClosed() {
            this.f30321b.onClosed();
        }

        @Override // com.ironsource.V8
        public void onUIReady() {
            if (Qf.this.f30317f) {
                this.f30321b.onUIReady();
            } else {
                Qf.this.f30312a.postDelayed(new Runnable() { // from class: com.ironsource.dj
                    @Override // java.lang.Runnable
                    public final void run() {
                        Qf.b.a(this.f31498b);
                    }
                }, 500L);
            }
        }
    }

    public Qf(@NotNull TestSuiteActivity testSuiteActivity, @NotNull V8 v82, @NotNull JSONObject jSONObject, @Nullable String str) {
        tn.p.k(testSuiteActivity, ActivityChooserModel.ATTRIBUTE_ACTIVITY);
        tn.p.k(v82, "uiLifeCycleListener");
        tn.p.k(jSONObject, "jsonData");
        this.f30312a = new Handler(testSuiteActivity.getMainLooper());
        this.f30313b = new WebView(testSuiteActivity);
        this.f30314c = new WebView(testSuiteActivity);
        this.f30315d = jSONObject;
        a(testSuiteActivity, v82, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(Qf qf2, String str) {
        tn.p.k(qf2, "this$0");
        tn.p.k(str, "$script");
        IronSourceNetworkBridge.webviewLoadUrl(qf2.f30314c, SafeDKWebAppInterface.f52902f + str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(V8 v82) {
        tn.p.k(v82, "$uiLifeCycleListener");
        v82.onUIReady();
    }

    @SuppressLint({"SetJavaScriptEnabled", "AddJavascriptInterface"})
    private final void a(TestSuiteActivity testSuiteActivity, final V8 v82, String str) {
        WebView webView = this.f30313b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f30314c.getSettings().setJavaScriptEnabled(true);
        IronSourceNetworkBridge.onAddedJavascriptInterface(this.f30314c, new Ff(new C4375zf(testSuiteActivity, this.f30312a), new b(v82), this), "TestSuiteBridge");
        this.f30314c.setWebViewClient(new a(str));
        if (str != null && str.length() != 0) {
            IronSourceNetworkBridge.webviewLoadUrl(this.f30314c, str);
        }
        WebView webView2 = this.f30313b;
        if (webView2 != null) {
            IronSourceNetworkBridge.webviewLoadUrl(webView2, "javascript:document.write(atob(''))");
        }
        this.f30312a.postDelayed(new Runnable() { // from class: com.ironsource.bj
            @Override // java.lang.Runnable
            public final void run() {
                Qf.e(this.f31073b);
            }
        }, 2000L);
        this.f30312a.postDelayed(new Runnable() { // from class: com.ironsource.cj
            @Override // java.lang.Runnable
            public final void run() {
                Qf.a(v82);
            }
        }, 5000L);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void e(Qf qf2) {
        tn.p.k(qf2, "this$0");
        qf2.f30317f = true;
    }

    @NotNull
    public final String a(@NotNull String str, @NotNull List<? extends Object> list) {
        tn.p.k(str, "methodName");
        tn.p.k(list, "methodArgs");
        String str2 = "TestSuiteJs." + str;
        String strB = Pf.f30178a.b(list);
        if (strB.length() <= 0) {
            return str2 + "()";
        }
        return str2 + "(" + strB + ")";
    }

    public final void a() {
        b();
        this.f30314c.destroy();
    }

    public final void a(@NotNull final String str) {
        tn.p.k(str, "script");
        this.f30312a.post(new Runnable() { // from class: com.ironsource.aj
            @Override // java.lang.Runnable
            public final void run() {
                Qf.a(this.f31001b, str);
            }
        });
    }

    @Override // com.ironsource.Gf
    public void a(@NotNull String str, @NotNull LevelPlay.AdFormat adFormat, @NotNull List<? extends Object> list) {
        tn.p.k(str, "callbackName");
        tn.p.k(adFormat, "adFormat");
        tn.p.k(list, "args");
        if (this.f30316e) {
            a(a("onCallbackReceived", cn.w.p(str, adFormat.getValue(), list)));
        }
    }

    public final void b() {
        WebView webView = this.f30313b;
        if (webView != null) {
            webView.destroy();
            this.f30313b = null;
        }
    }

    @NotNull
    public final WebView c() {
        return this.f30314c;
    }

    @Nullable
    public final WebView d() {
        return this.f30313b;
    }
}
