...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    private T scaleOnlyTransform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation, boolean z10) {
        T t10 = z10 ? (T) transform(downsampleStrategy, transformation) : (T) optionalTransform(downsampleStrategy, transformation);
    ...
    public T apply(@NonNull BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().apply(baseRequestOptions);
        }
    ...
    public T decode(@NonNull Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().decode(cls);
        }
    ...
    public T diskCacheStrategy(@NonNull DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().dontTransform();
        }
        this.transformations.clear();
        int i10 = this.fields & (-2049);
        this.isTransformationRequired = false;
        this.isTransformationAllowed = false;
        this.fields = (i10 & (-131073)) | 65536;
        this.isScaleOnlyOrNoTransform = true;
        return (T) selfOrThrowIfLocked();
    }
    ...
    public T encodeQuality(@IntRange(from = 0, to = 100) int i10) {
        return (T) set(BitmapEncoder.COMPRESSION_QUALITY, Integer.valueOf(i10));
    }
    ...
    public T error(@DrawableRes int i10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().error(i10);
        }
        this.errorId = i10;
    ...
    public T error(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().error(drawable);
        }
    ...
    public T fallback(@DrawableRes int i10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().fallback(i10);
        }
        this.fallbackId = i10;
    ...
    public T fallback(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().fallback(drawable);
        }
    ...
    public T frame(@IntRange(from = 0) long j10) {
        return (T) set(VideoDecoder.TARGET_FRAME, Long.valueOf(j10));
    }
    ...
    public T onlyRetrieveFromCache(boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().onlyRetrieveFromCache(z10);
        }
    ...
    public final T optionalTransform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    public T override(int i10, int i11) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().override(i10, i11);
        }
    ...
    public T placeholder(@DrawableRes int i10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().placeholder(i10);
        }
    ...
    public T placeholder(@Nullable Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().placeholder(drawable);
        }
    ...
    public T priority(@NonNull Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().priority(priority);
        }
    ...
    public <Y> T set(@NonNull Option<Y> option, @NonNull Y y10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().set(option, y10);
        }
    ...
    public T signature(@NonNull Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().signature(key);
        }
    ...
    public T sizeMultiplier(@FloatRange(from = 0.0d, to = 1.0d) float f10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().sizeMultiplier(f10);
        }
    ...
    public T skipMemoryCache(boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().skipMemoryCache(true);
        }
    ...
    public T theme(@Nullable Resources.Theme theme) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().theme(theme);
        }
        this.theme = theme;
        this.fields |= 32768;
        return (T) selfOrThrowIfLocked();
    ...
    public T timeout(@IntRange(from = 0) int i10) {
        return (T) set(HttpGlideUrlLoader.TIMEOUT, Integer.valueOf(i10));
    }
    ...
    public T transform(@NonNull Transformation<Bitmap> transformation, boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().transform(transformation, z10);
        }
    ...
    public final T transform(@NonNull DownsampleStrategy downsampleStrategy, @NonNull Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    public <Y> T transform(@NonNull Class<Y> cls, @NonNull Transformation<Y> transformation, boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().transform(cls, transformation, z10);
        }
    ...
    public T transform(@NonNull Transformation<Bitmap>... transformationArr) {
        return transformationArr.length > 1 ? (T) transform((Transformation<Bitmap>) new MultiTransformation(transformationArr), true) : transformationArr.length == 1 ? (T) transform(transformationArr[0]) : (T) selfOrThrowIfLocked();
    }
    ...
    public T useAnimationPool(boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().useAnimationPool(z10);
        }
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo4281clone().useUnlimitedSourceGeneratorsPool(z10);
        }
