...
public final class SingleRequest<R> implements Request, SizeReadyCallback, ResourceCallback {
    ...
    private void onLoadFailed(GlideException glideException, int i10) {
        ...
        synchronized (this.requestLock) {
            ...
            if (logLevel <= i10) {
                Log.w(GLIDE_TAG, "Load failed for " + this.model + " with size [" + this.width + VastAttributes.HORIZONTAL_POSITION + this.height + C3978d4.j.f31385e, glideException);
                if (logLevel <= 4) {
    ...
    private void onResourceReady(Resource<R> resource, R r10, DataSource dataSource, boolean z10) {
        ...
        if (this.glideContext.getLogLevel() <= 3) {
            Log.d(GLIDE_TAG, "Finished loading " + r10.getClass().getSimpleName() + " from " + dataSource + " for " + this.model + " with size [" + this.width + VastAttributes.HORIZONTAL_POSITION + this.height + "] in " + LogTime.getElapsedMillis(this.startTime) + " ms");
        }
        boolean z11 = true;
        this.isCallingCallbacks = true;
        try {
            ...
            if (list != null) {
                ...
                while (it.hasNext()) {
                    zOnResourceReady |= it.next().onResourceReady(r10, this.model, this.target, dataSource, zIsFirstReadyResource);
                }
            } else {
                zOnResourceReady = false;
            }
            RequestListener<R> requestListener = this.targetListener;
            if (requestListener == null || !requestListener.onResourceReady(r10, this.model, this.target, dataSource, zIsFirstReadyResource)) {
                z11 = false;
            }
            if (!(z11 | zOnResourceReady)) {
                this.target.onResourceReady(r10, this.animationFactory.build(dataSource, zIsFirstReadyResource));
            }
            this.isCallingCallbacks = false;
            notifyLoadSuccess();
            GlideTrace.endSectionAsync(TAG, this.cookie);
    ...
    public String toString() {
        ...
        return super.toString() + "[model=" + obj + ", transcodeClass=" + cls + C3978d4.j.f31385e;
    }
}
