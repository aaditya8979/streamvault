...
public abstract class c7 {
    ...
    public static boolean a(String str) {
        ...
        String[] strArrSplit = str.split("~", -1);
        return (strArrSplit.length == 2 && "1".equals(strArrSplit[0])) ? Pattern.matches("(\\d+(\\.\\d+)*)?", strArrSplit[1]) : strArrSplit.length == 3 && "2".equals(strArrSplit[0]) && Pattern.matches("(\\d+(\\.\\d+)*)?", strArrSplit[1]) && strArrSplit[2].length() >= 3 && strArrSplit[2].startsWith("dv.") && Pattern.matches("(\\d+(\\.\\d+)*)?", strArrSplit[2].substring(3));
    }
