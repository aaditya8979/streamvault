...
public enum AVTransportErrorCode {
    ...
    RECORD_FORMAT_NOT_SUPPORTED(IronSourceError.ERROR_NT_INSTANCE_LOAD_TIMEOUT, "The storage format of the currently loaded media is not supported for recording"),
    ...
    ILLEGAL_SEEK_TARGET(IronSourceError.ERROR_NT_INSTANCE_LOAD_EMPTY_ADAPTER, "The specified seek target is not specified in terms of the seek mode, or is not present on the media"),
    PLAYMODE_NOT_SUPPORTED(IronSourceError.ERROR_NT_INSTANCE_INIT_EXCEPTION, "The specified play mode is not supported by the device"),
    ...
    CONTENT_BUSY(IronSourceError.ERROR_NT_LOAD_NO_CONFIG, "The resource is already being played by other means"),
    RESOURCE_NOT_FOUND(716, "The specified resource cannot be found in the network"),
    INVALID_INSTANCE_ID(IronSourceError.ERROR_NT_INSTANCE_LOAD_EMPTY_SERVER_DATA, "The specified instanceID is invalid for this AVTransport");

    private int code;
    private String description;

    AVTransportErrorCode(int i10, String str) {
        this.code = i10;
        this.description = str;
    }

    public static AVTransportErrorCode getByCode(int i10) {
        for (AVTransportErrorCode aVTransportErrorCode : values()) {
            if (aVTransportErrorCode.getCode() == i10) {
