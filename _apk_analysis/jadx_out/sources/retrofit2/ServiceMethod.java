    ...
    public static final class Builder<T, R> {
        ...
        private CallAdapter<T, R> createCallAdapter() {
            ...
            try {
                ...
                throw methodError(e10, "Unable to create call adapter for %s", genericReturnType);
            }
        }
        ...
        private Converter<ResponseBody, T> createResponseConverter() {
            try {
                ...
                throw methodError(e10, "Unable to create converter for %s", this.responseType);
            }
        }
        ...
        private RuntimeException methodError(Throwable th2, String str, Object... objArr) {
            return new IllegalArgumentException(String.format(str, objArr) + "\n    for method " + this.method.getDeclaringClass().getSimpleName() + "." + this.method.getName(), th2);
        }

        private RuntimeException parameterError(int i10, String str, Object... objArr) {
            return methodError(str + " (parameter #" + (i10 + 1) + ")", objArr);
        }

        private RuntimeException parameterError(Throwable th2, int i10, String str, Object... objArr) {
            return methodError(th2, str + " (parameter #" + (i10 + 1) + ")", objArr);
        }
        ...
        private ParameterHandler<?> parseParameter(int i10, Type type, Annotation[] annotationArr) {
            ParameterHandler<?> parameterHandler = null;
            for (Annotation annotation : annotationArr) {
                ParameterHandler<?> parameterAnnotation = parseParameterAnnotation(i10, type, annotationArr, annotation);
                if (parameterAnnotation != null) {
                    if (parameterHandler != null) {
                        throw parameterError(i10, "Multiple Retrofit annotations found, only one allowed.", new Object[0]);
                    }
            ...
            throw parameterError(i10, "No Retrofit annotation found.", new Object[0]);
        }

        private ParameterHandler<?> parseParameterAnnotation(int i10, Type type, Annotation[] annotationArr, Annotation annotation) {
            if (annotation instanceof Url) {
                if (this.gotUrl) {
                    throw parameterError(i10, "Multiple @Url method annotations found.", new Object[0]);
                }
                if (this.gotPath) {
                    throw parameterError(i10, "@Path parameters may not be used with @Url.", new Object[0]);
                }
                if (this.gotQuery) {
                    throw parameterError(i10, "A @Url parameter must not come after a @Query", new Object[0]);
                }
                if (this.relativeUrl != null) {
                    throw parameterError(i10, "@Url cannot be used with @%s URL", this.httpMethod);
                }
                this.gotUrl = true;
                if (type == HttpUrl.class || type == String.class || type == URI.class || ((type instanceof Class) && "android.net.Uri".equals(((Class) type).getName()))) {
                    return new ParameterHandler.RelativeUrl();
                }
                throw parameterError(i10, "@Url must be okhttp3.HttpUrl, String, java.net.URI, or android.net.Uri type.", new Object[0]);
            }
            if (annotation instanceof Path) {
                if (this.gotQuery) {
                    throw parameterError(i10, "A @Path parameter must not come after a @Query.", new Object[0]);
                }
                if (this.gotUrl) {
                    throw parameterError(i10, "@Path parameters may not be used with @Url.", new Object[0]);
                }
                if (this.relativeUrl == null) {
                    throw parameterError(i10, "@Path can only be used with relative url on @%s", this.httpMethod);
                }
                this.gotPath = true;
            ...
            if (annotation instanceof Query) {
                ...
                throw parameterError(i10, rawType.getSimpleName() + " must include generic type (e.g., " + rawType.getSimpleName() + "<String>)", new Object[0]);
            }
            if (annotation instanceof QueryName) {
                ...
                throw parameterError(i10, rawType2.getSimpleName() + " must include generic type (e.g., " + rawType2.getSimpleName() + "<String>)", new Object[0]);
            }
            if (annotation instanceof QueryMap) {
                Class<?> rawType3 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType3)) {
                    throw parameterError(i10, "@QueryMap parameter type must be Map.", new Object[0]);
                }
                Type supertype = Utils.getSupertype(type, rawType3, Map.class);
                if (!(supertype instanceof ParameterizedType)) {
                    throw parameterError(i10, "Map must include generic types (e.g., Map<String, String>)", new Object[0]);
                }
                ...
                throw parameterError(i10, "@QueryMap keys must be of type String: " + parameterUpperBound, new Object[0]);
            }
            if (annotation instanceof Header) {
                ...
                throw parameterError(i10, rawType4.getSimpleName() + " must include generic type (e.g., " + rawType4.getSimpleName() + "<String>)", new Object[0]);
            }
            if (annotation instanceof HeaderMap) {
                Class<?> rawType5 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType5)) {
                    throw parameterError(i10, "@HeaderMap parameter type must be Map.", new Object[0]);
                }
                ...
                throw parameterError(i10, "@HeaderMap keys must be of type String: " + parameterUpperBound2, new Object[0]);
            }
            if (annotation instanceof Field) {
                if (!this.isFormEncoded) {
                    throw parameterError(i10, "@Field parameters can only be used with form encoding.", new Object[0]);
                }
                ...
                throw parameterError(i10, rawType6.getSimpleName() + " must include generic type (e.g., " + rawType6.getSimpleName() + "<String>)", new Object[0]);
            }
            if (annotation instanceof FieldMap) {
                if (!this.isFormEncoded) {
                    throw parameterError(i10, "@FieldMap parameters can only be used with form encoding.", new Object[0]);
                }
                Class<?> rawType7 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType7)) {
                    throw parameterError(i10, "@FieldMap parameter type must be Map.", new Object[0]);
                }
                ...
                throw parameterError(i10, "@FieldMap keys must be of type String: " + parameterUpperBound3, new Object[0]);
            }
            if (!(annotation instanceof Part)) {
                if (!(annotation instanceof PartMap)) {
                    ...
                    if (this.isFormEncoded || this.isMultipart) {
                        throw parameterError(i10, "@Body parameters cannot be used with form or multi-part encoding.", new Object[0]);
                    }
                    if (this.gotBody) {
                        throw parameterError(i10, "Multiple @Body method annotations found.", new Object[0]);
                    }
                    try {
                        ...
                        throw parameterError(e10, i10, "Unable to create @Body converter for %s", type);
                    }
                }
                if (!this.isMultipart) {
                    throw parameterError(i10, "@PartMap parameters can only be used with multipart encoding.", new Object[0]);
                }
                this.gotPart = true;
                Class<?> rawType8 = Utils.getRawType(type);
                if (!Map.class.isAssignableFrom(rawType8)) {
                    throw parameterError(i10, "@PartMap parameter type must be Map.", new Object[0]);
                }
                ...
                if (String.class == parameterUpperBound4) {
                    ...
                    if (MultipartBody.Part.class.isAssignableFrom(Utils.getRawType(parameterUpperBound5))) {
                        throw parameterError(i10, "@PartMap values cannot be MultipartBody.Part. Use @Part List<Part> or a different value type instead.", new Object[0]);
                    }
                ...
                throw parameterError(i10, "@PartMap keys must be of type String: " + parameterUpperBound4, new Object[0]);
            }
            if (!this.isMultipart) {
                throw parameterError(i10, "@Part parameters can only be used with multipart encoding.", new Object[0]);
            }
            ...
            if (strValue5.isEmpty()) {
                if (!Iterable.class.isAssignableFrom(rawType9)) {
                    if (rawType9.isArray()) {
                        ...
                        throw parameterError(i10, "@Part annotation must supply a name or use MultipartBody.Part parameter type.", new Object[0]);
                    }
                ...
                throw parameterError(i10, rawType9.getSimpleName() + " must include generic type (e.g., " + rawType9.getSimpleName() + "<String>)", new Object[0]);
            }
            Headers headersOf = Headers.of("Content-Disposition", "form-data; name=\"" + strValue5 + "\"", "Content-Transfer-Encoding", part.encoding());
            if (!Iterable.class.isAssignableFrom(rawType9)) {
                if (!rawType9.isArray()) {
                    if (MultipartBody.Part.class.isAssignableFrom(rawType9)) {
                        throw parameterError(i10, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                    }
        ...
        private void validatePathName(int i10, String str) {
            if (!ServiceMethod.PARAM_NAME_REGEX.matcher(str).matches()) {
                throw parameterError(i10, "@Path parameter name must match %s. Found: %s", ServiceMethod.PARAM_URL_REGEX.pattern(), str);
            }
            if (!this.relativeUrlParamNames.contains(str)) {
                throw parameterError(i10, "URL \"%s\" does not contain \"{%s}\".", this.relativeUrl, str);
            }
        }
        ...
        public ServiceMethod build() {
            ...
            for (int i10 = 0; i10 < length; i10++) {
                ...
                if (Utils.hasUnresolvableType(type)) {
                    throw parameterError(i10, "Parameter type must not include a type variable or wildcard: %s", type);
                }
                Annotation[] annotationArr = this.parameterAnnotationsArray[i10];
                if (annotationArr == null) {
                    throw parameterError(i10, "No Retrofit annotation found.", new Object[0]);
                }
                this.parameterHandlers[i10] = parseParameter(i10, type, annotationArr);
            }
