...
public final class ListenerSet<T> {
    ...
    private final CopyOnWriteArraySet<a<T>> listeners;
    ...
    private ListenerSet(CopyOnWriteArraySet<a<T>> copyOnWriteArraySet, Looper looper, Clock clock, IterationFinishedEvent<T> iterationFinishedEvent, boolean z10) {
        ...
        this.handler = clock.createHandler(looper, new Handler.Callback() { // from class: li.b
            @Override // android.os.Handler.Callback
            public final boolean handleMessage(Message message) {
                return this.f73841b.handleMessage(message);
            }
        });
    ...
    public void queueEvent(final int i10, final Event<T> event) {
        ...
        this.queuedEvents.add(new Runnable() { // from class: li.a
    ...
    public void remove(T t10) {
        verifyCurrentThread();
        for (a<T> aVar : this.listeners) {
            if (aVar.listener.equals(t10)) {
                aVar.release(this.iterationFinishedEvent);
